...
type Product struct {
	...
	Discount_price sql.NullInt64 `gorm:"type:int(11)"`
	Date           *time.Time    `gorm:"type:DEFAULT;NOT NULL"`
	Main_cat       sql.NullInt64 `gorm:"type:int(11)"`
	Sub_cat        sql.NullInt64 `gorm:"type:int(11)"`
	Qty            sql.NullInt64 `gorm:"type:int(11)"`
...
type ProductAtt struct {
	ID     int           `gorm:"column:id;type:int(11);AUTO_INCREMENT;PRIMARY_KEY`
	PID    sql.NullInt64 `gorm:"column:p_id;type:int(11)`
...
type Media struct {
	ID            int           `gorm:"column:id;type:int(11);AUTO_INCREMENT;PRIMARY_KEY`
	PageID        sql.NullInt64 `gorm:"column:page_id;type:int(11)`
	...
	Author        sql.NullInt64 `gorm:"type:int(11)`
	OrderID       sql.NullInt64 `gorm:"column:order_id;type:int(11)`
}
